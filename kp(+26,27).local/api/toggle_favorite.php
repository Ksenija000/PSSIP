...
if ($exists) {
    $db->prepare("DELETE FROM favorites_products WHERE user_id = ? AND product_id = ?")->execute([$user_id, $product_id]);
    $action = 'removed';
} else {
    $db->prepare("INSERT INTO favorites_products (user_id, product_id, added_at) VALUES (?, ?, NOW())")->execute([$user_id, $product_id]);
    $action = 'added';
}
