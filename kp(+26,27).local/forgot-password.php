<?php
// ============================================
// forgot-password.php - ЗАПРОС СБРОСА ПАРОЛЯ (ДЕМО-РЕЖИМ)
// ============================================
session_start();
...
$message = '';
$error = '';
$reset_link = ''; // Для отображения ссылки
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email'])) {
    ...
    $stmt = $db->prepare("SELECT id, fio FROM users WHERE email = ?");
    $stmt->execute([$email]);
    $user = $stmt->fetch();
    ...
    if ($user) {
        ...
        // Сохраняем в БД (токен храним в хэшированном виде!)
        ...
        $reset_link = "http://{$_SERVER['HTTP_HOST']}/reset-password.php?token=" . urlencode($token);
        
        // ВАРИАНТ 1: Показываем ссылку прямо на странице
        $message = 'ДЕМО-РЕЖИМ: Используйте эту ссылку для сброса пароля:';
        
        // ВАРИАНТ 2: Записываем в файл (раскомментировать, если нужно)
        // file_put_contents('reset_links.log', date('Y-m-d H:i:s') . " - {$user['fio']} - {$reset_link}\n", FILE_APPEND);
...
// Получаем категории для меню (как на других страницах)
$categories = $db->query("SELECT * FROM categories ORDER BY name")->fetchAll();

// Проверка авторизации для счетчиков
...
// Код авторизации/регистрации (как на других страницах) - можно скопировать
// ...
?>
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <title>ARTOBJECT | Восстановление пароля</title>
    <style>
        /* ============================================ */
        /* СТИЛИ - КАК НА ДРУГИХ СТРАНИЦАХ */
        /* ============================================ */
        * {
            margin: 0;
            padding: 0;
    ...
    <section class="page-header">
        <div class="container">
            <div class="page-header-content">
                <h1>Восстановление <span class="text-gradient">пароля</span></h1>
            </div>
        </div>
    </section>
            ...
            <div class="reset-card">
                ...
                <?php if ($reset_link): ?>
                    <div class="reset-link-box">
                        <p><strong>Ссылка для сброса пароля (DEMO):</strong></p>
                        <a href="<?php echo $reset_link; ?>"><?php echo $reset_link; ?></a>
                        <p style="margin-top: 10px; font-size: 0.9rem;">Нажмите на ссылку или скопируйте её в браузер</p>
                    </div>
                <?php endif; ?>
                ...
                <div class="back-link">
                    <a href="index.php"><i class="fas fa-arrow-left"></i> Вернуться ко входу</a>
                </div>
            </div>
        </div>
    </section>
