...
if ($token) {
    // Ищем пользователя с таким токеном (проверяем по хэшу)
    $stmt = $db->prepare("SELECT id, reset_token, reset_token_expiry FROM users WHERE reset_token IS NOT NULL");
    $stmt->execute();
    $users = $stmt->fetchAll();
    
    foreach ($users as $user) {
        if (password_verify($token, $user['reset_token'])) {
            // Токен верный, проверяем срок действия
    ...
    if (!$user_id) {
        $error = 'Недействительная ссылка для восстановления.';
    }
}

// Обработка смены пароля
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password']) && $user_id) {
    $password = $_POST['password'];
...
<head>
    ...
    <title>ARTOBJECT | Сброс пароля</title>
    <style>
        /* ============================================ */
    ...
    <section class="page-header">
        <div class="container">
            <div class="page-header-content">
                <h1>Сброс <span class="text-gradient">пароля</span></h1>
            </div>
        </div>
    </section>
